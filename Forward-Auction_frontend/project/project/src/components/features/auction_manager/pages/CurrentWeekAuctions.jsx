...
import { getCurrentWeekAuctions } from '../service/auctionservice';

const CurrentWeekAuction = () => {
  const [weekAuctions, setWeekAuctions] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getCurrentWeekAuctions();
        setWeekAuctions(data);
      } catch (err) {
        console.error("Error fetching week's auctions:", err);
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="table-scroll">
            <table className="bidder-table live-auction-table">
              ...
              <tbody style={{ fontSize: '9.52px' }}>
                {weekAuctions.map((auction, index) => (
...
export default CurrentWeekAuction ;
